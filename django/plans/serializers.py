...
class PlanPaymentSerializer(serializers.ModelSerializer):
    class Meta:
        model = PlanPayment
        fields = [
            'id', 'total', 'currency', 'variant', 'status',
            'token', 'created', 'modified'
        ]
...
class UserSubscriptionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'end_date', 'is_expired', 'latest_payment', 'created_at'
        ]
    
    def get_latest_payment(self, obj):
        payment = PlanPayment.objects.filter(subscription=obj).order_by('-created').first()
        if payment:
