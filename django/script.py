...
def download_font_files_from_css(css_content, template_suffix):
    ...
    for i, font_url in enumerate(font_urls):
        ...
        if not os.path.exists(font_path):
            print(f"  Downloading font file {i+1}/{len(font_urls)} for {template_suffix}...")
            font_response = requests.get(font_url)
            with open(font_path, 'wb') as f:
                f.write(font_response.content)
        else:
            print(f"  ⏭️  Font file {i+1} for {template_suffix} already exists, skipping...")
        
        # Update CSS to use local fonts
        css_file = f'static/css/google-fonts-{template_suffix}.css'
        with open(css_file, 'r', encoding='utf-8') as f:
            css_content_updated = f.read()
        css_content_updated = css_content_updated.replace(font_url, f'../fonts/{font_filename}')
