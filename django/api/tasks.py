...
def generate_and_save_resume_task(self, user_id, input_text, job_description, language, ats_result=None):
    ...
    logger.info(f"Task {task_id}: generate_and_save_resume_task ENTERED for user_id: {user_id}.")  # Added user_id
    try:
        ...
        logger.info(f"Task {task_id}: Calling AI service for resume generation for user_id: {user_id}")
        ...
        if not generated_resume_yaml_str:
            logger.error(f"Task {task_id}: AI service did not return generated resume YAML for user_id: {user_id}.")
            raise Ignore("AI service did not return generated resume YAML.")  # Fail, don't retry

        # Parse the YAML
        generated_resume_data = yaml.safe_load(generated_resume_yaml_str)
        if not generated_resume_data:
            logger.error(f"Task {task_id}: Failed to parse generated resume YAML for user_id: {user_id}.")
            # Log the problematic YAML
            logger.error(f"Problematic YAML string for task {task_id}:\n---\n{generated_resume_yaml_str}\n---")
        ...
        # Find the user
        try:
            user = User.objects.get(id=user_id)  # Use get() for clarity, handle DoesNotExist
        ...
        logger.info(f"Task {task_id}: Successfully saved resume with id {new_resume.id} for user_id: {user_id}")
        return {"status": "SUCCESS", "resume_id": new_resume.id}

    # --- Specific, RETRYABLE Exceptions ---
    except requests.exceptions.Timeout as exc:
        logger.warning(f"Task {task_id}: Timeout calling AI service for user_id {user_id}: {exc}. Retrying...")
        ...
        if exc.response is not None and 400 <= exc.response.status_code < 500:
            logger.error(f"Task {task_id}: Non-retryable client error {exc.response.status_code} from AI service for user_id {user_id}: {exc}")
            raise Ignore(f"AI Service Client Error: {exc.response.status_code}")  # Fail, don't retry
        else:
            logger.warning(f"Task {task_id}: RequestException calling AI service for user_id {user_id}: {exc}. Retrying...")
    ...
    # --- Specific, NON-RETRYABLE Exceptions (Caught by type) ---
        ...
        logger.error(f"Task {task_id}: Data processing error (YAML/Value/Key) for user_id {user_id}: {e}")
    ...
    # --- Catch-all for UNEXPECTED errors (NON-RETRYABLE) ---
        ...
        logger.exception(f"Task {task_id}: UNEXPECTED error for user_id {user_id}: {e}")  # Use logger.exception to include traceback
        raise Ignore(f"Unexpected task error: {e}")  # Fail immediately, don't retry code bugs
...
def generate_resume_data_task(self, input_text, job_description, language, ats_result=None):
    ...
    # --- Specific, RETRYABLE Exceptions ---
        ...
        logger.warning(f"Task {task_id}: Timeout calling AI service: {exc}. Retrying...")
        raise self.retry(exc=exc)  # Retry on timeout

    except requests.exceptions.RequestException as exc:
        # Retry only on non-4xx errors (e.g., connection errors, 5xx server errors from AI)
        if exc.response is not None and 400 <= exc.response.status_code < 500:
            logger.error(f"Task {task_id}: Non-retryable client error {exc.response.status_code} from AI service: {exc}")
            raise Ignore(f"AI Service Client Error: {exc.response.status_code}")  # Fail, don't retry
        else:
            logger.warning(f"Task {task_id}: RequestException calling AI service: {exc}. Retrying...")
            raise self.retry(exc=exc)  # Retry on other connection/server errors
    ...
    # --- Catch-all for UNEXPECTED errors (NON-RETRYABLE) ---
        ...
        logger.exception(f"Task {task_id}: UNEXPECTED error: {e}")  # Use logger.exception to include traceback
        raise Ignore(f"Unexpected task error: {e}")  # Fail immediately, don't retry code bugs
